...
class HiUCD(Dataset):
    def __init__(self, root, transforms=None):
        self.im1_fps, self.im2_fps, self.gt_fps = group_files(
            root, ['image/2017/9/*.png', 'image/2018/9/*.png','mask_merge/2017_2018/9/*.png']
        )
        assert len(self.im1_fps) == len(self.gt_fps)
        assert len(self.im2_fps) == len(self.gt_fps)
        self.im_fps = self.im1_fps+self.im2_fps
        self.gt_fps=self.gt_fps+self.gt_fps
...
class HiUCDLoader_single(er.ERDataLoader):
    def __init__(self, config):
        super(HiUCDLoader_single, self).__init__(config)
