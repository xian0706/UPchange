...
def import_config(config_name, model_dir, prefix='configs'):
    cfg_path = '{}.{}'.format(prefix, config_name)
    m = importlib.import_module(name=cfg_path)
    os.makedirs(model_dir, exist_ok=True)
    shutil.copy(cfg_path.replace('.', '/') + '.py', os.path.join(model_dir, 'config.py'))
...
def adjust_learning_rate(optimizer, i_iter, cfg):
    if i_iter < cfg.preheat_step:
        lr = lr_warmup(cfg.learning_rate.params.base_lr, i_iter, cfg.preheat_step)
    else:
        lr = lr_poly(cfg.learning_rate.params.base_lr, i_iter, cfg.learning_rate.params.max_iters, cfg.learning_rate.params.power)
...
def adjust_learning_rate_D(optimizer, i_iter, cfg):
    lr = lr_poly(cfg.learning_rate_D, i_iter, cfg.learning_rate.params.max_iters, cfg.learning_rate.params.power)
